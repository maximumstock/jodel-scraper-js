...
class DynamicScraper {
	...
	scrape() {
		if (this._stop === true) {
			logger.info(`Stopping ${this.getDescription()}`);
			return;
		}

		logger.info(`Scraping ${this.getDescription()}`);

		this.authorize()
			.then(() => {
				// at this point `this.auth` should be populated properly
				return api.get_feed(this.auth.access_token, this.feed);
			})
			.then(feed => {
				...
				// schedule next invocation of `this.scrape()`
				setTimeout(() => {
					return this.scrape();
	 ...
	 */
	getDescription() {
		return `${this.feed}, ${this.location.latitude} - ${this.location.longitude} (${this.location.name || ''})`;
	}
