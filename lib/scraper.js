...
class DynamicScraper {
   ...
   * @param {string} device_uid - A registered device_uid
   * @param {string} feed - The feed to scrape (`discussed`, `popular`, default: `recent`)
   ...
   * to scrape for. `name` and `country_code` are not required, but help with 
   * specifying ethe location when
   * 
   ...
   *
   */
  constructor(device_uid, feed, location, options = {}) {
  ...
  scrape() {
    ...
    this.authorize()
      ...
      .catch(error => {
        logger.error(`${this.getDescription()}: Encountered an error`);
        logger.error(error);
