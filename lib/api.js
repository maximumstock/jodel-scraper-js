...
function generate_hmac(token, method, url, body) {
	...
	const hmac = crypto.createHmac('sha1', 'schnitzel');
 ...
 * @param {string} token 
 * @param {string} feed 
 * @param {object} query - Optional parameters such as `limit` and `after` 
 */
function get_feed_batch(token, feed, query = {}) {
	...
	return new Promise((resolve, reject) => {
		...
		const url = `${config.JODEL_API_V2}/posts/location${mapped_feed ? '/' : ''}${mapped_feed}`;
	...
	return new Promise((resolve, reject) => {
		...
		function callback(response) {
			try {
				...
				if (jodels.length < config.MAX_JODELS_PER_REQUEST) {
					...
					// what Jodels to skip
					const options = {
						after: jodels[jodels.length - 1].post_id
					};
					return get_feed_batch(token, feed, options)
						.then(callback)
						.catch(reject);
				}
			} catch (e) {
