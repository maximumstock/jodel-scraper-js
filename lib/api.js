...
function generate_deviceuid(static = false) {
  if(static) {
    ...
    for(let i = 0; i < length; i++) {
      uid += alphabet[parseInt(Math.random() * 1000) % alphabet.length];
    }
...
function default_headers(options = {}) {
  ...
  const headers = {
    ...
    'X-Authorization': options.hmac ? `HMAC ${options.hmac}` : null,
 ...
 */
function request_token(device_uid, latitude, longitude, location_name = '', country_code = '', location_accuracy = 0) {
...
/**
 * `get_feed_batch` version that uses the `/api/v3/posts/combo` endpoint
 */
// function get_feed_batch_combo(token, query = {}) {}

/**
 * `get_feed` version that uses the `/api/v3/posts/combo` endpoint
 */
// function get_feed_combo(token, query = {}) {}
...
module.exports = {
  ...
  get_feed: get_feed,
  generate_deviceuid: generate_deviceuid
}
