...
function generate_hmac(token, method, url, body) {
  ...
  const raw = method.toUpperCase() + '%' + url.host + '%' + 443 + '%' + url.path + '/%' + (token || '') + '%' + url.query + '%' + body;
  const raw2 = method.toUpperCase() + '@' + url.path + '%' + url.host + '%' + 443 + '%' + url.path + '/%' + (token || '') + '%' + url.query + '%' + body;

  const hmac = crypto.createHmac('sha1', 'hyTBJcvtpDLSgGUWjybbYUNKSSoVvMcfdjtjiQvf');
...
function generate_deviceuid() {
  ...
  // As of 2017-07-27: I'm pretty sure the static device_uid is still necessary
  // const length = 64;
